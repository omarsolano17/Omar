...
export function useFechas() {
  ...
  const getDate = (value) => {
    if (value == "date") {
      ...
      return `${fecha}`; // .charAt(0).toUpperCase() + value.slice(1)
    }
    if (value == "time") {
      let hora = "";
  ...
  const getPrescripcion = (hcatd, sinDisposicion, soloDosisTotal) => {
    ...
    if (item.RAZONNEC === 2) {
      ...
      const dosisTotal =
        (item.CANTIDADAM + item.CANTIDADPM) *
        (item.DURACION * 1 > 24 ? (item.DURACION * 1) / 24 : item.DURACION);
      ...
      if (item.DURACION * 1 > 0) {
        if (item.PORSUBIR) {
          ...
          texto += ` por ${
            ...
            item.TIPODURACION === "D" || item.DURACION * 1 >= 24
              ? `d韆${item.DURACION * 1 > 24 ? "s" : ""}`
              : "horas"
          }.`;
        }
      }
  ...
  const getPrescripcionEnOrdenes = (
    hcatd,
    tipoDuracion,
    sinDisposicion,
    soloDosisTotal
